 ...
 * @access  Private
 */
export const createCollaborationGoal = catchAsync(async (req: Request, res: Response) => {
  const {
    title,
    description,
    participants,
  }: { title: string; description: string; participants: string[] } = sanitize(req.body);
 ...
 * @access  Private
 */
export const getUserCollaborationGoals = catchAsync(async (req: Request, res: Response) => {
  const userId = req.user?.id;
 ...
 * @access  Private
 */
export const deleteCollaborationGoal = catchAsync(async (req: Request, res: Response) => {
 ...
 * @access  Private
 */
export const addParticipant = catchAsync(async (req: Request, res: Response) => {
  const { goalId, participantId }: { goalId: string; participantId: string } = sanitize(req.body);
  const userId = req.user?.id;
  ...
  if (!goalId || !participantId) {
    sendResponse(res, 400, false, "Goal ID and participant ID are required");
    return;
  }
...
/**
 * @desc    Remove a participant from a collaboration goal
 * @route   POST /api/collaboration-goals/remove-participant
 * @access  Private
 */
export const removeParticipant = catchAsync(async (req: Request, res: Response) => {
  ...
  if (goal.createdBy.toString() !== userId) {
    sendResponse(res, 403, false, "Not authorized to remove participants from this goal");
    return;
  }

  goal.participants = goal.participants.filter((id) => id.toString() !== participantId);
  await goal.save();

  sendResponse(res, 200, true, "Participant removed successfully", { goal });
});
