...
Cypress.Commands.add("navigateToPayments", () => {
   ...
   */
  Cypress.Commands.add("simulateSuccessfulPayment", (amount: number, method: string) => {
   ...
   */
  Cypress.Commands.add("verifyPaymentHistory", (payments: Array<{ id: number; date: string; amount: number; method: string }>) => {
    ...
    payments.forEach(({ date, amount, method }) => {
      ...
      cy.contains(".payment-history-item", `$${amount.toFixed(2)}`).should("be.visible");
   ...
   */
  Cypress.Commands.add("simulatePaymentValidationError", (field: string, errorMessage: string) => {
