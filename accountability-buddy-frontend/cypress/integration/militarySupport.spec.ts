...
describe('Military Support Section', () => {
    ...
    it('should notify users of new messages', () => {
      ...
      cy.get('.new-message-notification').should('be.visible').and('contain.text', 'You have new messages');
    });
  
    it('should support clearing chat history', () => {
      // Mock API response for clearing chat history
      cy.intercept('DELETE', '/api/military-support/messages', { statusCode: 200 }).as('clearMessages');
