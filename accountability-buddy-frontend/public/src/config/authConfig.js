...
const authConfig = {
    ...
    setRefreshToken: (token) => localStorage.setItem(authConfig.refreshTokenKey, token),
    removeRefreshToken: () => localStorage.removeItem(authConfig.refreshTokenKey),
  
    // Token Expiry Handling
    getTokenExpiry: () => parseInt(localStorage.getItem(authConfig.tokenExpiryKey), 10),
    setTokenExpiry: (expiry) => localStorage.setItem(authConfig.tokenExpiryKey, expiry.toString()),
    isTokenExpired: () => {
