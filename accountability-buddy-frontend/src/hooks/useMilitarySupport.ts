...
const useMilitarySupport = () => {
  ...
  const fetchResources = useCallback(async () => {
    ...
    setError(null);
    try {
      const response = await axios.get<MilitaryResource[]>('/military-support/resources');
  ...
  const addResource = useCallback(async (newResource: Omit<MilitaryResource, 'id'>) => {
    ...
    setError(null);
    try {
      const response = await axios.post<MilitaryResource>('/military-support/resources', newResource);
  ...
  const updateResource = useCallback(async (id: string, updatedResource: Partial<MilitaryResource>) => {
    setLoading(true);
    setError(null);
    try {
      ...
      setResources((prev) =>
        prev.map((resource) => (resource.id === id ? response.data : resource))
      );
    } catch (err) {
