...
describe("AnimatedButton Component", () => {
  ...
  it("applies the correct styles based on the variant prop", () => {
    const { rerender } = render(<AnimatedButton label="Primary" variant="primary" />);
    ...
    expect(screen.getByRole("button", { name: /outline/i })).toHaveClass("outline");
  });
  ...
  it("applies the correct size based on the size prop", () => {
    ...
    expect(screen.getByRole("button", { name: /medium/i })).toHaveClass("medium");
