...
describe('Dashboard Component', () => {
  ...
  it('renders recent activities', () => {
    render(<Dashboard {...mockProps} />);
    expect(
      screen.getByText(/completed the 'fitness challenge' goal/i)
    ).toBeInTheDocument();
    expect(
      screen.getByText(/collaborated with alex on 'project x'/i)
    ).toBeInTheDocument();
    expect(
      screen.getByText(/started the 'healthy eating' goal/i)
    ).toBeInTheDocument();
  });
