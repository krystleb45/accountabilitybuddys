...
export const initGA = (
  trackingId: string = 'G-XXXXXXXXXX',
  debug: boolean = false
): void => {
  if (!trackingId) {
...
export const trackEvent = ({
  ...
  console.log(
    `Event tracked: Category - ${category}, Action - ${action}, Label - ${label}, Value - ${value}`
  );
};
...
export const trackConversion = (
  ...
  console.log(
    `Conversion tracked: Type - ${conversionType}, Details - ${details}`
  );
};
