...
export const initMixpanel = (
  token: string = 'YOUR_MIXPANEL_TOKEN',
  debug: boolean = false
): void => {
  if (!token) {
...
export const incrementUserProperty = (
  ...
  if (!userId || !property) {
    console.warn(
      'User property increment called without a user ID or property name.'
    );
    return;
  }
  ...
  console.log(
    `User property incremented: ID - ${userId}, Property - ${property}, Incremented By - ${incrementBy}`
  );
};
