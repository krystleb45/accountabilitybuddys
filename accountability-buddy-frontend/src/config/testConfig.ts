...
const testConfig: TestConfig = {
  ...
  testEnvironment:
    (process.env.TEST_ENV as 'development' | 'staging' | 'production') ||
    'development',
  apiBaseUrl: process.env.TEST_API_BASE_URL || 'http://localhost:5000', // Default API URL
  ...
  logLevel:
    (process.env.TEST_LOG_LEVEL as 'info' | 'warn' | 'error' | 'debug') ||
    'info',
